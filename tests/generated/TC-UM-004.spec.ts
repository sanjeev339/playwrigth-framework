...
test("TC-UM-004: User Management", async ({ page }) => {
  ...
  await test.step("Step 2: Select User", async () => {
    await selectCustomDropdown(page, () => page.getByRole("cell", { name: /adithya j sanjeevkumar\.m00@gmail\.com/i }), "");
  });

  await test.step("Step 3: Click Deactivate", async () => {
    await page.getByText(/Deactivate/i).click();
    await page.waitForLoadState('domcontentloaded').catch(() => undefined);
  });
});
