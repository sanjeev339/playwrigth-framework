...
test("TC-UM-003: User Management", async ({ page }) => {
  ...
  await test.step("Step 2: Click sanjeevkumar.m00@gmail.com", async () => {
    await page.getByText(/sanjeevkumar\.m00@gmail\.com/i).click();
    await page.waitForLoadState('domcontentloaded').catch(() => undefined);
  });

  await test.step("Step 3: Click Edit", async () => {
    await page.getByRole("button", { name: /Edit/i }).click();
    await page.waitForLoadState('domcontentloaded').catch(() => undefined);
  });

  await test.step("Step 4: Select Role", async () => {
