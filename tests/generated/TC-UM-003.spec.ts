...
test("TC-UM-003: User Management", async ({ page }) => {
  ...
  await test.step("Step 5: Click Role", async () => {
    await page.locator("xpath=/html/body/div[2]/div[1]/div[2]/div[1]/div[2]/div[4]/div[1]/div[1]/div[1]").click();
    await expect(page).toHaveURL(/user-detail/i, { timeout: 15000 });
  });

  await test.step("Step 6: Select Role", async () => {
    await selectCustomDropdown(page, () => page.locator("xpath=/html/body/div[2]/div[1]/div[2]/div[1]/div[2]/div[4]/div[1]/div[1]/div[1]"), String(payload["Role"]));
    await expect(page).toHaveURL(/user-detail/i, { timeout: 15000 });
  });
