...
export async function generateTests(options: GenerateTestsOptions = {}): Promise<GenerationReport> {
  ...
  const testsDir = path.resolve(outputDir, '..');
  await ensureFrameworkFiles(testsDir);
...
async function generateScenarioIndependently(input: {
  ...
  try {
    ...
    stage = 'prompt-build';
    const testsDir = path.resolve(input.outputDir, '..');
    const moduleName = scenario.module ?? 'UnknownModule';
    await ensureModulePageObject(testsDir, moduleName);
...
function validateGeneratedReconTest(code: string, scenario: Scenario, reconActions: ReconAction[]): void {
  ...
  if (!code.includes(`import { ${pageClassName} }`) && !code.includes(`import {${pageClassName}}`)) {
    throw new Error(`Generated test missing page object import for ${pageClassName}`);
  }
  if (!code.includes('import { buildPayload } from')) {
    throw new Error('Generated test missing Faker buildPayload import');
  }
...
const ensuredPageObjects = new Set<string>();

async function ensureModulePageObject(testsDir: string, moduleName: string): Promise<string> {
  const className = moduleName.replace(/[^a-zA-Z0-9]/g, '') + 'Page';
  
  if (ensuredPageObjects.has(className)) {
    return className;
  }
  ensuredPageObjects.add(className);

  const filePath = path.join(testsDir, 'pages', `${className}.ts`);

  if (!(await fs.pathExists(filePath))) {
    const skeleton = `import type { Page } from '@playwright/test';
import { BasePage } from './base/BasePage';

/**
 * Page Object: ${moduleName}
 * Auto-generated by OrchestAI. Extend with module-specific methods.
 * This file is created ONCE and NOT overwritten on subsequent pipeline runs.
 */
...
`;
    ...
    await writeTextFileAtomic(filePath, skeleton);

    const indexPath = path.join(testsDir, 'pages', 'index.ts');
...
async function ensureFrameworkFiles(testsDir: string): Promise<void> {
  const frameworkTestsDir = path.resolve(__dirname, '../../tests');
  
  if (!(await fs.pathExists(frameworkTestsDir))) {
    logger.warn(`Framework tests base dir not found at ${frameworkTestsDir}. Cannot copy base layers.`);
    return;
  }
  ...
  for (const layer of layers) {
    ...
    if (await fs.pathExists(srcPath) && !(await fs.pathExists(destPath))) {
      ...
      logger.info(`Initialized base layer: ${layer} in project`);
    }
  }

  // Ensure .auth directory exists for storageState
  await fs.ensureDir(path.join(testsDir, '.auth'));
}
