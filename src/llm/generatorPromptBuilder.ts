...
export function buildGeneratorPrompt(input: {
  ...
  return truncate(
    [
      ...
      '12. When selecting options from a dropdown (like Role), NEVER use `getByRole("option")` because custom dropdowns often use custom divs or checkboxes. Instead, ALWAYS use `await page.getByText(String(payload["Role"]), { exact: true }).last().click();`. The `.last()` is critical to ensure you select the option inside the newly opened popup and not a background table element.',
      '13. Do not write assertions that strictly look for literal strings from the "Expected Results" column. Instead, assert that a toast notification is visible using `await expect(page.locator(".p-toast, .toast, [role=\\"alert\\"], [role=\\"status\\"]").first()).toBeVisible({ timeout: 10000 });`.',
